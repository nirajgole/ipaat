...
-- Alphanumeric data either fixed at n symbols or up to n symbols. It’s not possible to do 
-- arithmetic on this data.
...
-- queries? 
-- B) What different kinds of punctuation marks can be used inside date values 
-- in order to separate month, day, and year?
-- A) number signs. 
-- B) Dashes and slashes are the correct separators for dates when you 
...
-- FROM Carsale
SELECT FORMAT(Hire_Date, 'dddd, mmmm dd, yyyy')
FROM employee
...
-- FROM Carsale
SELECT FORMAT(Hire_Date, 'dd mm yy')
FROM employee
